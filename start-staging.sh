...
export CHROMA_DB_DIR="chroma_db_staging"
export NEXT_PUBLIC_API_PORT=8005
...
cleanup() {
  echo ""
  log_info "Shutting down staging servers..."
  ...
  log_success "All staging servers stopped. Goodbye!"
  exit 0
}
...
# 3. Start Backend
log_info "Starting Backend (FastAPI) on port 8005..."
cd backend
...
# 4. Start Frontend
log_info "Starting Frontend (Next.js) on port 3005..."
cd frontend
npm run dev -- -H 0.0.0.0 -p 3005 &
FRONTEND_PID=$!
cd ..

log_success "Staging servers started!"
log_info "- Frontend: http://localhost:3005"
log_info "- Backend API: http://localhost:8005"
log_info "- Database Directory: $CHROMA_DB_DIR"
