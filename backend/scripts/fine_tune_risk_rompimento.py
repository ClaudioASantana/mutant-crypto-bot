"""
Script de Ajuste Fino de Parâmetros de Risco (SL/TP) para a estratégia Rompimento.

Executa uma varredura (sweep) refinada sobre sl_multiplier e tp_multiplier para
a estratégia Rompimento, usando o BacktestService com BacktestSimulatorImpl.
"""
import os
import sys
import json
...
async def main():
    print("🚀 Iniciando otimização refinada de parâmetros de risco para Rompimento...")

    # Baixa o histórico uma vez
    ...
    # Varre parâmetros para a estratégia Rompimento
    results = await sweep_parameters("Rompimento", history)

    print("\n🏆 === TOP 10 Rompimento Refinado ===")
    for i, r in enumerate(results, 1):
        print(f"{i}. SL:{r['sl']} TP:{r['tp']} | PNL:${r['pnl']} | WR:{r['wr']}% | DD:{r['dd']}% | Trades:{r['trades']}")

    # Salva resultados completos
    with open("fine_tuned_risk_results_rompimento.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n💾 Resultados completos salvos em 'fine_tuned_risk_results_rompimento.json'")
