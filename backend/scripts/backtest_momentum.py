"""
Script de Backtest da Estratégia de Rompimento (Momentum Breakout).

Executa um backtest da nova estratégia "Rompimento" usando o BacktestSimulatorImpl
...
async def run_backtest(strategy: str, history: list):
    ...
    # Vamos usar SL=1.5, TP=3.0 como ponto de partida (valores medianos do ajuste fino)
...
async def main():
    print("🚀 Iniciando backtest da estratégia de Rompimento (Momentum Breakout)...")
    ...
    momentum_result = await run_backtest("Rompimento", history)

    print("\n🏆 === Resultado do Backtest de Rompimento ===")
    print(f"PNL: ${momentum_result['pnl']}")
    print(f"Win Rate: {momentum_result['wr']}%")
    print(f"Drawdown Máximo: {momentum_result['dd']}%")
    print(f"Total de Trades: {momentum_result['trades']}")
    print(f"Wins: {momentum_result['wins']}, Losses: {momentum_result['losses']}")

    # Salva resultado
    with open("backtest_momentum_result.json", "w") as f:
        json.dump(momentum_result, f, indent=2)
    print("\n💾 Resultado salvo em 'backtest_momentum_result.json'")
