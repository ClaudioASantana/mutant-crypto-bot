"""
Script de Ajuste Fino de Parâmetros de Risco (SL/TP) para a estratégia BB_MA_MACD.
"""
import os
import sys
import json
...
async def sweep_parameters(strategy: str, history: list):
    ...
    sl_multipliers = [1.0, 1.2, 1.5, 1.8, 2.0, 2.2, 2.5, 3.0]
    tp_multipliers = [1.0, 1.2, 1.5, 1.8, 2.0, 2.5, 3.0, 4.0, 5.0]
    results = []
...
async def main():
    print("🚀 Iniciando otimização de risco para BB_MA_MACD...")
    ...
    results = await sweep_parameters("BB_MA_MACD", history)

    print("\n🏆 === TOP 10 BB_MA_MACD Refinado ===")
    for i, r in enumerate(results[:10], 1):
        print(f"{i}. SL:{r['sl']} TP:{r['tp']} | PNL:${r['pnl']} | WR:{r['wr']}% | DD:{r['dd']}% | Trades:{r['trades']}")

    with open("fine_tuned_risk_bb_ma_macd.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n💾 Resultados completos salvos em 'fine_tuned_risk_bb_ma_macd.json'")
