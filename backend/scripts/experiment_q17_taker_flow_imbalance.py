#!/usr/bin/env python3
"""Q17 — taker flow imbalance — série longa da Binance Vision.

Pré-registro: docs/estrategias/q17-taker-flow-imbalance-design-20260822.md

Objetivo: testar se extremos de fluxo agressivo comprador/vendedor (taker flow)
carregam continuação curta do preço acima do controle incondicional.
...
def _ms_to_ts(vals) -> pd.Series:
    """Converte epoch-ms para timestamp UTC, sempre em resolução ns."""
...
def detect_events(df: pd.DataFrame) -> dict:
    ...
    for i in range(len(df)):
        row = df.iloc[i]
        ts = df.index[i]
        if not np.isfinite(row["taker_pct"]):
            continue
        if row["taker_pct"] >= TAKER_HIGH_Q:
            events_e1.append((i, ts))
        elif row["taker_pct"] <= TAKER_LOW_Q:
...
def main():
    print("🧪 Q17 — taker flow imbalance — série longa (Camada A histórica)")
    print("   E1: taker buy imbalance extremo → continuação (+) | E2: taker sell imbalance extremo → continuação (-) | custo 10bp")
    ...
    print(f"Eventos E1 (taker buy imbalance extremo):  {len(events['E1'])}")
    print(f"Eventos E2 (taker sell imbalance extremo): {len(events['E2'])}")
