"""
Script de Ajuste Fino de SL/TP para a estratégia BB_MA_MACD (v2).
"""
import os
import sys
import json
...
async def sweep_parameters(history: list):
    print("\n🔍 Ajustando SL/TP para BB_MA_MACD v2...")
...
async def main():
    print("🚀 Iniciando otimização de SL/TP para BB_MA_MACD v2...")
    ...
    print("\n🏆 === TOP 10 BB_MA_MACD v2 ===")
    for i, r in enumerate(results, 1):
        print(f"{i}. SL:{r['sl']} TP:{r['tp']} | PNL:${r['pnl']} | WR:{r['wr']}% | DD:{r['dd']}% | Trades:{r['trades']}")

    with open("fine_tuned_risk_results_bb_ma_macd_v2.json", "w") as f:
        json.dump(results, f, indent=2)
    print("\n💾 Resultados salvos em 'fine_tuned_risk_results_bb_ma_macd_v2.json'")
